...
const TournamentCard: React.FC<TournamentCardProps> = ({ tournament }) => {
  const accent = GAME_COLORS[tournament.game] || '#ff7a00';
  return (
      ...
      >
        ...
        <div className="relative z-10">
          <div className="flex items-center mb-2">
            {tournament.logoUrl && (
              <img src={tournament.logoUrl} alt={tournament.name} className="w-10 h-10 mr-3 rounded" />
            )}
            <span className="font-extrabold text-xl text-white">{tournament.name}</span>
            {tournament.status === 'live' && <span className="ml-2"><LiveIndicator /></span>}
          </div>
          <div className="text-sm text-light mb-1 font-bold">{tournament.game}</div>
          <div className="flex justify-between text-xs text-light">
            <span>{new Date(tournament.startDate).toLocaleDateString()} - {new Date(tournament.endDate).toLocaleDateString()}</span>
            <span className="uppercase font-bold">{tournament.status}</span>
          </div>
        </div>
      </motion.div>
    </Link>
